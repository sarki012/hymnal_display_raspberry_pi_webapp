...
import socket
import time
from password import WEB_PASSWORD
...
def unquote_plus(s):
    """
    A simple URL decoder for form data that handles '+' for space and %HH encoding.
    This is a lightweight replacement for urllib.parse.unquote_plus.
    """
    if not s:
        return ""
    ...
    for i in range(1, len(res)):
        try:
            ...
            # If decoding fails (e.g., malformed '%'), prepend '%' back
            res[i] = '%' + res[i]
    
    # Join the parts and replace '+' with space
    decoded_str = "".join(res)
    return decoded_str.replace('+', ' ')

def recv_full_request(client):
    """
    Reads a full HTTP request from the socket: loops recv() until the
    header terminator is seen, then reads exactly Content-Length more
    bytes for the body. A single recv(1024) isn't enough once real
    browsers add enough headers (cookies, sec-ch-ua, etc.) to push the
    body past the first 1024 bytes.
    """
    request = b""
    ...
    return header_part.decode('utf-8', 'ignore'), body_part.decode('utf-8', 'ignore')
...
def serve_webpage(ip):
    ...
    try:
        with open("index.html", "r") as f_keypad:
            keypad_html = f_keypad.read()
        with open("login.html", "r") as f_login:
            login_html = f_login.read()
    except OSError as e:
        print(f"Error: Cannot open HTML file. {e}")
        return

    # A simple set to store IPs of authenticated clients
    ...
    while True:
        ...
        try:
            ...
            header_str, body_str = recv_full_request(client)
            ...
            if "GET /login" in request_line:
                client.send('HTTP/1.0 200 OK\r\nContent-Type: text/html\r\n\r\n')
                client.send(login_html)

            elif "POST /login" in request_line:
                # Find the password in the POST body
                submitted_pass = ""
                try:
                    ...
                    for part in parts:
                        if part.startswith("password="):
                            ...
                            submitted_pass = unquote_plus(part.split('=', 1)[1]).strip()
                except (IndexError, ValueError):
                    pass # Could not parse, submitted_pass remains ""

                # Debugging: Print what is being compared
                print(f"Comparing submitted='{submitted_pass}' with expected='{WEB_PASSWORD}'")
                if submitted_pass == WEB_PASSWORD:
                    print(f"Successful login from IP: {client_ip}")
                    authenticated_clients.add(client_ip)
                    # Redirect to the main page
                    client.send('HTTP/1.0 302 Found\r\nLocation: /\r\n\r\n')
                else:
                    print(f"Failed login attempt from IP: {client_ip}")
                    # Show login page again with an error
                    error_html = login_html.replace('</h1>', '</h1><p class="error">Incorrect Password</p>')
                    client.send('HTTP/1.0 401 Unauthorized\r\nContent-Type: text/html\r\n\r\n')
                    client.send(error_html)

            # --- Protected Routes ---
            # Check if client is authenticated for all other routes
            elif client_ip not in authenticated_clients:
                print(f"Unauthenticated access from {client_ip}, redirecting to login.")
                client.send('HTTP/1.0 302 Found\r\nLocation: /login\r\n\r\n')

            elif "GET /on?code=" in request_line: # Authenticated route
                try:
            ...
            elif "GET / " in request_line: # Authenticated route
                client.send('HTTP/1.0 200 OK\r\nContent-Type: text/html\r\n\r\n')
                client.send(keypad_html)

            elif "GET /off" in request_line: # Authenticated route
